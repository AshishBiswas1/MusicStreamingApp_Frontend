...
const Profile = ({ onBack }) => {
  ...
  const [recommendedInput, setRecommendedInput] = useState(
    user && Array.isArray(user.recommended) ? user.recommended.join(', ') : ''
  );
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Prepare recommended array from comma-separated input
      const newRecommendedArr = (recommendedInput || '')
        .split(',')
        .map((s) => s.trim())
        .filter((s) => s.length > 0);

      const currentRecommendedArr = Array.isArray(user?.recommended)
        ? user.recommended
        : [];
      ...
      const recommendedChanged =
        JSON.stringify(newRecommendedArr) !==
        JSON.stringify(currentRecommendedArr);

      if (!nameChanged && !imageChanged && !recommendedChanged) {
        setError('No changes to save');
        setUpdating(false);
        return;
      }

      // If recommended changed, include it in the FormData as JSON
      if (recommendedChanged) {
        formData.append('recommended', JSON.stringify(newRecommendedArr));
      }

      const response = await authService.updateMe(formData);

      // Update user context with returned user
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 pb-32 relative z-50 flex flex-col items-center">
      <SongUpload
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    ...
                    <div className="mt-3">
                      ...
                      <textarea
                        ...
                        placeholder="Comma-separated recommended items (e.g. song slugs or keywords)"
                        className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                        rows={3}
                      />
                      <p className="text-gray-500 text-xs mt-1">
                        Enter comma-separated values for your recommended list.
                      </p>
                    </div>
                  ...
                  <>
                    ...
                    <div className="mt-4">
                      ...
                      {Array.isArray(user?.recommended) &&
                      user.recommended.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {user.recommended.map((rec, idx) => (
                            <span
                              key={idx}
